...
- **Engagements** tab (left sidebar) — search-by-name, status filter, and paginated table. Defaults to **grouped-by-family** view: each row represents one engagement family (the latest revision), with a chevron that expands to show every prior revision as sub-rows in **descending revision order** (R{N-1}, …, R0 below the latest parent); the parent row's status reflects the latest revision. A "Show all revisions as separate rows" toggle switches to a flat view (one row per engagement) and persists across sessions in `localStorage`. The list is always sorted by document number, descending — biggest DD number on top — and column headers are non-interactive. The doc-number cell strips any `-R{n}` suffix; the R-chip on each row is the revision differentiator. Cancelled revisions are rendered muted in the expanded sub-rows.
